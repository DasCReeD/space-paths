...
**Where we are:** The original bug (buggy verticality collision — a 2D-on-rails engine with height bolted on) is **FIXED and committed**. The engine was fully rewritten into a unified multi-span column model; true 3D verticality (bridges/overpasses you drive over AND under) works end-to-end, verified in-browser. Tunnels (ride-on-top + side walls) and two clip-through bugs (floor tunneling, corner grounding) all fixed. **734 unit tests + editor tests = 742 total, all green.**

**Branch:** `collision-engine-redesign` (2 commits ahead of the pre-existing work):
...
**NEXT STEPS (in order):**
1. **P4.3** — editor authoring UI for spans (add/edit/remove per cell; ramp tool writes spans; `activePlaneHeight` selects span; `EditSpansCommand`). sonnet. Files: editor.js, editorCommands.js, inGameEditor.js. Keep ponytail-minimal.
2. **P5** — worldBuilder solver + procedural multi-span generation. **opus + ponytail** (hard-algorithmic: the DFS `solveLevel` must be upgraded to multi-span using heightfield.js queries). See P5.1–P5.3.
3. **P6** — update docs/level_format_guide.md (`spans`), architecture.md, module-map.md; final regression.

**PENDING (not blocking):** Visual review of editor.html multi-span rendering (P4.2) — open editor, load a `spans` cell, confirm the overpass slab shows a visible underside (not a pillar).
...
**KEY ARCHITECTURE / GOTCHAS:** substep cap = `min(0.5, ship.height*0.9)` (must stay < ship height or floor tunnels). Grounding is footprint-aware (`supportSurface` halfW/halfL). Tunnels = road span + rideable roof (thin slab, top at baseY+archHeight) + partial-width wall-leg spans, all built run-aware in `heightfield.js addTunnelStructure`. Partial-width spans = `Span.xMin/xMax` (collision.js + queries respect them). Directional (swept) push-out in `collision.js findContact` (back leading edge to entry face — handles obstacles thinner than the ship). physics works in active-level-LOCAL Y (flat ground=0); app.js Flow/Tower H=25 wrap unchanged. `_buildGridFromCollidables` in physics.js is a transitional TEST-ONLY shim (remove when ramp tests feed real level data).
