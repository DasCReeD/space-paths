// Ad-hoc visual-verification harness for the biome neon texture sets.
// Clean elevated 3/4 shots (sim frozen, cockpit + chrome hidden) of the demo
// level + the first level of each of the 10 generated biomes, plus in-biome
// variants, so the road/obstacle textures can be judged directly.
// Capture technique borrowed from tools/generate_level_thumbs.mjs.
// Usage: PORT=3002 node playtests/biome_shots.mjs   (dev server must be running)
...
const url = 'http://localhost:' + (process.env.PORT || '3002');
const outDir = path.join(__dirname, 'biome_shots');
fs.mkdirSync(outDir, { recursive: true });

// pack idx: generated pack list index == level_index - 61; standard idx 0 == demo.
const targets = [
  { pack: 'standard', idx: 0, label: 'demo' },
  { pack: 'generated', idx: 0, label: 'void_61' },
  { pack: 'generated', idx: 1, label: 'void_62' },
  { pack: 'generated', idx: 3, label: 'ridge_64' },
  { pack: 'generated', idx: 6, label: 'thrill_67' },
  { pack: 'generated', idx: 9, label: 'core_70' },
  { pack: 'generated', idx: 12, label: 'glitch_73' },
  { pack: 'generated', idx: 15, label: 'tundra_76' },
  { pack: 'generated', idx: 18, label: 'furnace_79' },
  { pack: 'generated', idx: 21, label: 'shallows_82' },
  { pack: 'generated', idx: 24, label: 'spire_85' },
  { pack: 'generated', idx: 27, label: 'pulse_88' },
];
...
const run = async () => {
  ...
  for (const t of targets) {
    try {
      ...
      // Freeze physics.update the instant the level starts so the auto-running ship
      // can't advance and die — but keep gameState 'playing' so rendering continues.
      ...
      // Freeze graphics.update too (keeps our manual camera), switch to an external
      // cam, hide chrome, and set a scenic elevated pose.
      await page.evaluate(() => {
